...
public class Main_Run_SameData {
	...
	public static void main (String [] args) throws Exception
	{
		...
		{
			run = "run"+running;
			RRHC_SameDataWriteResult.write52VF(vf52Data, running);
			//System.out.println(run);
			runExperiment (run, running, vf52Data);
		}
	}
	
	private static void runExperiment (String run, int running, double [][] vf52Data) throws Exception
	{
		int restartIter = 10;
		...
		RRHC_SameDataWriteResult.writetime(time, modelName, run, running);
	...
	public static void runRestart (double [][] vf52Data, Classifier model, String modelName, int restart, String run, int running) throws Exception
	{
		...
		for (int i=0; i<iter; i++)
		{
			...
			result = Register_Result.result(result, i, curAccuracy, newAccuracy, curCluster.size(), newCluster.size());
			newCluster.clear();
			System.out.println();
		...
		RRHC_SameDataWriteResult.writeResult(result, modelName, time, restart, run, running);
		RRHC_SameDataWriteResult.writeBundles(curCluster, restart, modelName, run, running);
