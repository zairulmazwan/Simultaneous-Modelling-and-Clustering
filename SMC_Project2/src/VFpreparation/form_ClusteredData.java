...
public class form_ClusteredData {
		...
		{	
			...
			{
				for (int j=1; j<=52; j++) //to read and calculate bundle VF each row of the record based on the clusters, start from 1 because PID is at 0 index
				{
					for (int c=0; c<cluster; c++) // to check the VF point in every clusters
					{
						...
						boolean vfpoint = VFCluster.get(c).contains(j-1); // to check VF location which bundle it belongs (true or false)
