...
public class Bundles_Creation {
		...
		public static int genRandomCluster (int lenghtVar)
		{
		
			int min_range = 2;
			int max_range = lenghtVar;
		...
		public static void addElement (ArrayList<ArrayList<Integer>> VFCluster, int var) // randomly adds VF point into a random chosen cluster
		{
		...
		public static ArrayList<ArrayList<Integer>> initialCluster (ArrayList<ArrayList<Integer>> iniCluster, int lenghtVar)
		{
			int rn =  genRandomCluster (lenghtVar-1); // get a random number for the outer arraylist cluster
			int var = lenghtVar-2;
			
			//System.out.println("No of var is : "+var);
			
			iniCluster = createEmptyCluster(rn); // create an empty cluster for the number of random number
			//System.out.println(iniCluster);
			addElement(iniCluster, var); // adds 52 VF points to a random clusters
			System.out.println(iniCluster);
